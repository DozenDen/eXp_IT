...
class Admin(db.Model):
    ...
    admin_login = db.Column(db.String(25))
...
def Admin_login():
    ...
    if not Admin.query.filter_by(admin_login=login).all():
        message = "Enter correct login!"
        return render_template('login.html', message=message)
    else:
        if not Admin.query.filter_by(admin_password=password).all():
...
def delete_article():
    ...
    for id1 in id_list:
        row = Posts.query.filter_by(id=id1).first()
